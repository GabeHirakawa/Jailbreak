...
public class GunGameDay(BasePlugin plugin, IServiceProvider provider)
  ...
  public override void Setup() {
    ...
    spawns =
      new ShuffleBag<Vector>(
        MapUtil.GetRandomSpawns(Utilities.GetPlayers().Count));
  ...
  override protected HookResult OnCanAcquire(DynamicHook hook) {
    ...
    if (WeaponTag.RIFLES.Contains(weapon))
      allowed = allowed.Union(WeaponTag.RIFLES).ToHashSet();

    if (WeaponTag.PISTOLS.Contains(weapon))
      allowed = allowed.Union(WeaponTag.PISTOLS).ToHashSet();
