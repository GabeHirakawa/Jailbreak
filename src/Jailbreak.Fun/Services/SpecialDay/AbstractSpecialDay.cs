...
public abstract class AbstractSpecialDay {
  ...
  protected AbstractSpecialDay(BasePlugin plugin, IServiceProvider provider) {
    Plugin   = plugin;
    Provider = provider;
  }
  ...
  private void doTeleports(SpecialDaySettings.TeleportType type,
    ...
    switch (type) {
      ...
      case SpecialDaySettings.TeleportType.RANDOM:
        spawnPositions =
          MapUtil.GetRandomSpawns(PlayerUtil.GetAlive().Count());
        break;
      case SpecialDaySettings.TeleportType.RANDOM_STACKED:
        spawnPositions = [MapUtil.GetRandomSpawns(1).First()];
        break;
      default:
        return;
    }
