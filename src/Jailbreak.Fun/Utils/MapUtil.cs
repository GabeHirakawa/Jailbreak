...
public static class MapUtil {
  ...
  public static void OpenCells() {
    OpenCells(Sensitivity.NAME_CELL_DOOR);
  }
  ...
  public static Sensitivity? OpenCells(
    ...
    while (true) {
      ...
      foreach (var button in allButtons
       .Select(cell => entityCache.TryGetValue((int)cell.Index, out var v) ? v : null)
  ...
  /// <summary>
  /// Simplified GetRandomSpawns without zone support.
  /// </summary>
  public static List<Vector> GetRandomSpawns(int count) {
  ...
  public static List<Vector> GetRandomSpawns(
    ...
    // TODO: Re-enable zone-based spawns when IZoneManager is migrated
    // if (zoneManager != null) { ... }
