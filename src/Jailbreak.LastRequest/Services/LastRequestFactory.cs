using CounterStrikeSharp.API.Core;
using Jailbreak.LastRequest.Enums;
using Jailbreak.LastRequest.Locale;
using Jailbreak.LastRequest.Services.Types;
using Jailbreak.LastRequest.Utils;

namespace Jailbreak.LastRequest.Services;

public class LastRequestFactory(ILastRequestManager manager,
  ILastRequestLocale messages, IBeamShapeFactory shapeFactory,
  BasePlugin plugin) : ILastRequestFactory {
  public AbstractLastRequest CreateLastRequest(CCSPlayerController prisoner,
    CCSPlayerController guard, LRType type) {
    return type switch {
      LRType.KNIFE_FIGHT => new KnifeFight(plugin, manager, messages,
        prisoner, guard),
      LRType.GUN_TOSS =>
        new GunToss(plugin, manager, messages, prisoner, guard),
      LRType.NO_SCOPE => new NoScope(plugin, manager, messages,
        prisoner, guard),
      LRType.SHOT_FOR_SHOT => new BulletForBullet(plugin, manager, messages,
        prisoner, guard, false),
      LRType.ROCK_PAPER_SCISSORS => new RockPaperScissors(plugin, manager,
        messages, prisoner, guard),
      LRType.COINFLIP => new Coinflip(plugin, manager, messages,
        prisoner, guard),
      LRType.RACE => new Race(plugin, manager, messages, shapeFactory,
        prisoner, guard),
      LRType.MAG_FOR_MAG => new BulletForBullet(plugin, manager, messages,
        prisoner, guard, true),
      _ => throw new ArgumentException("Invalid last request type: " + type,
        nameof(type))
    };
  }

  public bool IsValidType(LRType type) {
    return type switch {
      LRType.KNIFE_FIGHT         => true,
      LRType.GUN_TOSS            => true,
      LRType.NO_SCOPE            => true,
      LRType.SHOT_FOR_SHOT       => true,
      LRType.ROCK_PAPER_SCISSORS => true,
      LRType.COINFLIP            => true,
      LRType.RACE                => true,
      LRType.MAG_FOR_MAG         => true,
      _                          => false
    };
  }
}
