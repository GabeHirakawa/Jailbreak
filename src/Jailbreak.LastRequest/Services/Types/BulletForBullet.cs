...
public class BulletForBullet(BasePlugin plugin, ILastRequestManager manager,
  ...
  public override void Execute() {
    ...
    Plugin.AddTimer(5, () => {
      ...
      messages.B4bPlayerGoesFirst(shooter).ToChat(Prisoner, Guard);
    ...
    Plugin.AddTimer(60, () => {
      ...
      if (Guard.Health == Prisoner.Health) {
        ...
        messages.WinByReason(winner, "equal health");

        result = whosShot == Prisoner.Slot ?
          LRResult.GUARD_WIN :
          LRResult.PRISONER_WIN;
      } else {
        messages.WinByHealth(result == LRResult.GUARD_WIN ? Guard : Prisoner);
      }
