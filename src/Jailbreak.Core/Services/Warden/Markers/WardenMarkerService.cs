...
/// <summary>
/// Manages the warden marker placement and resizing.
/// Migrated from Jailbreak.Warden.Markers.WardenMarkerBehavior.
/// </summary>
public class WardenMarkerService : IMarkerService {
  ...
  /// <summary>
  /// Initialize - register the ping command listener.
  ...
  public HookResult OnPing(EventPlayerPing ev, GameEventInfo info) {
    ...
    if (settings == null) {
      ...
      settings = new Stubs.MarkerSettings(BeamShapeType.CIRCLE, Color.White);
    }
  ...
  private void ensurePlacedMarker(Stubs.MarkerSettings settings, Vector pos) {
